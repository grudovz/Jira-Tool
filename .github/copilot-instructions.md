...
- **Folding a gap directly into content vs. flagging it as a question**: fold it in only when the fix is mechanical/non-judgment — applying an already-established convention or an existing fallback pattern used elsewhere in the codebase/stories. Anything that actually requires a decision (exact wording, event granularity, scope) must stay an explicit open question instead, even if a plausible-looking default exists. Stay conservative here — a silently-folded assumption that turns out wrong is worse than an extra question, so bias toward flagging over guessing.
